		...
		<div className="h-full overflow-auto scrollbar-thin bg-background">
			...
			<div className="px-6 py-4 space-y-2 max-w-2xl">
				{connections.map((conn) => {
					...
					const logoColor =
						CONN_COLORS[conn.type] ?? "text-muted-foreground";
...
function TableGridView({
	...
	const [viewMode, setViewMode] = useState<"data" | "form" | "structure">(
		"data",
	);
	...
	const [cellModalFormat, setCellModalFormat] = useState<
		"Text" | "JSON" | "HTML"
	>("Text");
		...
		<div className="h-full flex flex-col bg-background overflow-hidden">
								...
								onClick={async () => {
									try {
										if (viewMode === "structure") {
											...
											// Fallback: refresh sidebar table list / trigger a high-level refresh
											await refreshTables(
												fn.connectionId,
											);
			...
			{contextMenuCell &&
				createPortal(
					...
					>
						{(() => {
							const { x, y, rowIdx, col, rowData } =
								contextMenuCell;
							const cellValue =
								col !== null ? rowData[col] : null;
							const menuW = 224;
							...
							return (
								...
								>
									...
									{item(
										...
										() =>
											col &&
											copyCellAsMarkdown(col, cellValue),
										undefined,
										false,
										!col,
									)}
					...
					>
						...
						>
							{(() => {
								...
								const colSize =
									table.getColumn(colId)?.getSize() ?? 150;
								const item = (
									label: string,
									action: () => void,
									shortcut?: string,
								...
								return [
									...
									item("Copy column name", () =>
										copyToClipboard(colId),
									),
									item("Copy as TSV for Excel", () =>
										copyColAsTSV(colId),
									),
									item("Copy as JSON", () =>
										copyColAsJSON(colId),
									...
									item("Copy as Markdown", () =>
										copyColAsMarkdown(colId),
									),
									item("Copy as SQL", () =>
										copyColAsSQL(colId),
									),
									item("Copy for IN statement", () =>
										copyColForIN(colId),
									),
									sep("s1"),
									item("Sort ascending", () =>
									...
									item("Resize all columns to match", () =>
										resizeAllToMatch(colSize),
									),
									item(
										"Resize all columns to fit content",
										() => resizeAllToFitContent(),
									),
									item(
										"Resize all columns to fixed width",
										() => setColumnSizing({}),
									),
									sep("s3"),
									item(`Hide ${colId}`, () =>
									...
									item("Open column filter", () =>
										openFilterForCol(colId),
									),
								];
							})()}
						</div>
			...
			>
				...
				>
					...
					<DialogDescription className="sr-only">
						Edit the cell value with syntax highlighting. Choose
						format and apply to save.
					</DialogDescription>
					{/* Header */}
					...
					>
						{cellModal && (
							<CodeMirror
								...
								onChange={(val) =>
									setCellModal({ ...cellModal, value: val })
								}
								theme={oneDark}
								extensions={[
									cellModalFormat === "JSON"
										? jsonLang()
										: cellModalFormat === "HTML"
											? htmlLang()
											: [],
									cellModalWrap
					...
					<div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-border/50 bg-[#1a1a1a]">
						...
						<Button
							...
							className="px-5"
							onClick={() =>
								cellModal && copyToClipboard(cellModal.value)
							}
						>
							Copy
						</Button>
				...
				<AlertDialogContent className="max-w-md">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>"{columnNullConfirmCol}"</strong> to NULL
				...
				<AlertDialogContent className="max-w-md">
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction
							...
							className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
						>
							{dropTableLoading ? "Dropping…" : "Drop Table"}
						</AlertDialogAction>
					</AlertDialogFooter>
				</AlertDialogContent>
			</AlertDialog>
			...
			<Dialog
				...
				onOpenChange={(o) => !o && setShowRenameTable(false)}
			>
				<DialogContent className="max-w-sm">
					...
					<div className="flex flex-col gap-3 py-2">
						<Input
							...
							onKeyDown={(e) =>
								e.key === "Enter" && executeRenameTable()
							}
					...
					<DialogFooter>
						<Button
							variant="outline"
							onClick={() => setShowRenameTable(false)}
						>
							Cancel
						</Button>
						<Button
				...
				<DialogContent className="max-w-md">
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={addColLoading || !addCol.name.trim()}
						>
						...
						<div className="flex flex-col gap-1.5">
							...
							<div className="flex flex-col gap-0.5 max-h-44 overflow-y-auto rounded-md border border-border bg-muted/20 p-1.5">
								{(structure?.columns ?? []).map((col) => {
									const order = createIdxDef.columns.indexOf(
										col.name,
									);
