...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (activeConnection) {
      setLoading(true);
      tauriApi.getDatabases(activeConnection.id)
        .then(setDatabases)
        .catch(console.error)
  ...
  useEffect(() => {
    if (activeConnection && activeDatabase) {
      setLoading(true);
      tauriApi.getTables(activeConnection.id, activeDatabase)
        .then(setTables)
        .catch(console.error)
        .finally(() => setLoading(false));
    }
  ...
  if (!activeConnection) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 text-center bg-sidebar-bg">
        ...
        <p className="text-xs font-bold text-text-muted uppercase tracking-widest">No Active Connection</p>
      </div>
    );
  }
      ...
      <div className="p-4 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 overflow-hidden">
            <div className="size-6 bg-blue-600 shrink-0 rounded flex items-center justify-center font-bold text-white text-[10px]">
              {activeConnection.type.substring(0, 2).toUpperCase()}
            </div>
            <span className="text-sm font-bold tracking-tight truncate">{activeConnection.name}</span>
          </div>
          <Plus 
            size={14} 
        ...
        <div className="px-0 relative group">
          <Combobox 
            ...
            onValueChange={(val) => setActiveDatabase(val as string)}
          >
            <ComboboxInput 
            ...
            <ComboboxContent className="bg-popover border border-white/5 shadow-2xl overflow-hidden rounded-xl min-w-50">
              <ComboboxList className="p-1 max-h-75">
                <ComboboxEmpty className="py-6 text-[11px] font-bold uppercase tracking-widest text-text-muted">No databases found</ComboboxEmpty>
                {databases.map(db => (
                  <ComboboxItem 
                    key={db} 
                    value={db}
        ...
        <div className="relative group">
          <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-blue-500 transition-colors" />
          <input 
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between px-2 mb-2">
             <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">
               {isLoading ? <Loader2 size={10} className="animate-spin" /> : `${tables.length} objects`}
             </span>
          </div>
          <div className="flex flex-col gap-0.5">
            {tables.map(table => (
              <div key={table.name} className="group">
                ...
                >
                  <ChevronDown size={12} className={activeTable === table.name ? '' : '-rotate-90 text-text-muted'} />
                  <Database size={13} className={activeTable === table.name ? 'text-blue-400' : 'text-text-muted'} />
                  <span className="text-[11px] font-semibold flex-1 truncate">{table.name}</span>
                </div>
                {activeTable === table.name && table.columns && (
                  <div className="ml-6 mt-1 flex flex-col gap-1 mb-2">
                    {table.columns.map(col => (
                      <div key={col.name} className="flex items-center justify-between pr-2 py-0.5 group/item cursor-default">
                        <span className="text-[10px] font-medium text-text-muted group-hover/item:text-text-secondary truncate">{col.name}</span>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[9px] font-mono text-zinc-500 uppercase">{col.type}</span>
                          {col.isPrimary && <span className="text-[8px] px-1 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded font-black leading-none py-0.5">PK</span>}
                        </div>
                      </div>
                    ))}
                  </div>
